...
const QRGenerator = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.get(`/qrcode/jwt?${params}`);
      
      setQrResult(response.data.qrCode);
      setUsage(response.data.usage);
      
      if (response.data.qrCode.cached) {
        toast.success('QR Code retrieved from cache!');
      } else {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-6">Generate QR Code</h2>
              ...
              <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span>Hourly:</span>
                    <span className={getUsageColor(usage.hourly.used, usage.hourly.limit)}>
                      {usage.hourly.used}/{usage.hourly.limit}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span>Daily:</span>
                    <span className={getUsageColor(usage.daily.used, usage.daily.limit)}>
                      {usage.daily.used}/{usage.daily.limit}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span>Monthly:</span>
                    <span className={getUsageColor(usage.monthly.used, usage.monthly.limit)}>
          ...
          {/* Result Section */}
          <div className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-6">QR Code Result</h2>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">{qrResult.name}</p>
                  </div>
                  
                  <div>
                    <label className="block text-sm font-medium text-gray-700">URL</label>
                    <div className="mt-1 flex items-center space-x-2">
                      <p className="text-sm text-gray-900 flex-1 truncate">{qrResult.url}</p>
                      <button
                  ...
                  <div className="flex items-center space-x-4 text-sm text-gray-500">
                    ...
                    <div>
                      Access Count: {qrResult.accessCount}
                    </div>
                  </div>
                </div>
